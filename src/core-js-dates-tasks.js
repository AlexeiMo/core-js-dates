...
function getTime(date) {
  ...
  const result = `${hours % 10 === hours ? `0${hours}` : hours}:${
...
function getNextFriday(date) {
  ...
  if (weekDay === 5) return new Date(date.getTime() + 86400000 * 7);
  if (weekDay === 6) {
    return new Date(date.getTime() + 86400000 * 6);
  }
  return new Date(date.getTime() + 86400000 * (5 - weekDay));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    (new Date(dateEnd).getTime() - new Date(dateStart).getTime()) / 86400000 + 1
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  if (startDate.getDay() / 6 !== 1) {
    if (startDate.getDay() === 0) weekendDayCount += 1;
    date += 6 - startDate.getDay();
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  let params = period.start.split('-');
  const startDate = new Date(params[2], params[1] - 1, params[0]);
  params = period.end.split('-');
  const endDate = new Date(params[2], params[1] - 1, params[0]);
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      const result = `${day % 10 === day ? `0${day}` : day}-${
